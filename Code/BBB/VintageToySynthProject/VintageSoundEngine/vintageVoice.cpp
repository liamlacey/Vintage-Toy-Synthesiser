...
VintageVoice::VintageVoice (uint8_t voice_num)
{
    ...
    sample[0].load("/usr/Alesis-Fusion-Bright-Acoustic-Piano-C2.wav");
    sample[1].load("/usr/Alesis-Fusion-Steel-String-Guitar-C4.wav");
    
    //show info on samples
    //printf("Summary:\n%s", sample.getSummary());
    
    //init other variables and objects
    for (uint8_t i = 0; i < NUM_OF_SAMPLES; i++)
    {
        //init sample position to the end so that it doesn't play on lauch
        sample[i].setPosition (1.0);
        sampleSpeed[i] = 1.0;
        sampleVeloAmp[i] = 1.0;
        
        envAmp[i].setAttack (0.);
        envAmp[i].setDecay (0.);
        envAmp[i].setSustain (1.);
        envAmp[i].setRelease (1000.);
    }
...
void VintageVoice::processAudio (double *output)
{
    ...
    //generate the amp evelope output
    for (uint8_t i = 0; i < NUM_OF_SAMPLES; i++)
    {
        envAmpOut[i] = envAmp[i].adsr (1.0, envAmp[i].trigger);
    }
    ...
    for (uint8_t i = 0; i < NUM_OF_SAMPLES; i++)
    {
        //just play the file once at the set speed/pitch, multipled by velo amp value and amp evelope value
        //UPDATE: envelopes not working for some reason - only two in ANY voice will play and then everything is silence!!
        sampleOut += (((sample[i].playOnce(sampleSpeed[i]) * sampleVeloAmp[i]) / (float)NUM_OF_SAMPLES) /** envAmpOut[i]*/);
    }
    
    //apply to output of all channels
...
void VintageVoice::processNoteMessage (bool note_status, uint8_t note_num, uint8_t note_vel)
{
    ...
    if (note_status == true)
    {
        //for each audio sample in the voice
        for (uint8_t i = 0; i < NUM_OF_SAMPLES; i++)
        {
            //============================
            ...
            //============================
            //very basic (and not even close) MIDI note to sample speed conversion
            //FIXME: do the proper equation for this
            if (note_num == 60)
                sampleSpeed[i] = 1.0;
            else if (note_num < 60)
                sampleSpeed[i] = scaleValue (note_num, 0, 59, 0.001, 0.99, 1.0);
            else
                sampleSpeed[i] = scaleValue (note_num, 61, 127, 1.001, 10.0, 1.0);
            
            //============================
            //set the sample velo amp from MIDI note velo value

            sampleVeloAmp[i] = scaleValue (note_vel, 0, 127, 0., 1., 1.);
            //sampleVeloAmp[i] = sampleVeloAmp[i] * sampleVeloAmp[i] * sampleVeloAmp[i];
        }
